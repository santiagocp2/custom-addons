...
class HelpdeskTicket(models.Model):
    ...
    sh_merge_ticket_ids = fields.Many2many(
        'helpdesk.ticket', relation='model_merge_helpdesk_ticket', column1="helpdesk", column2="ticket", string='Merge Tickets')
        ...
        if vals.get('partner_id'
            ...
            for rec in self:
                for template in rec.company_id.new_stage_id.mail_template_ids:
                    ...
                    if not rec.team_id or not rec.team_id.alias_id:
                        
                        email_values = {
                            'email_from': rec.company_id.email_formatted, 'email_to': rec.partner_id.email}                        

                    else:
                        
                        if rec.team_id.alias_id and rec.team_id.alias_id.alias_name and rec.team_id.alias_id.alias_domain:

                            alias_email = rec.team_id.alias_id.alias_name + \
                                '@'+rec.team_id.alias_id.alias_domain
                            alias_email = tools.formataddr(
                                (rec.team_head.name or u"False", alias_email or u"False"))
                            email_values = {'email_from': alias_email,
        ...
        if vals.get('team_id') and vals.get('team_head') and vals.get(
            ...
            if allocation_template:
                for rec in self:
                    ...
                    if not rec.team_id or not rec.team_id.alias_id:
                        email_values = {
                            'email_from': res.company_id.email_formatted, 'email_to': res.partner_id.email} 
                    else:
        ...
        if self.stage_id.sh_next_stage:
            ...
            if self.stage_id.mail_template_ids:
                for template in self.stage_id.mail_template_ids:
                    if not self.team_id or not self.team_id.alias_id:

                        email_values = {
                            'email_from': self.company_id.email_formatted, 'email_to': self.partner_id.email}
                        template.sudo().send_mail(self.id, force_send=True, email_values=email_values)

                    else:

                        if self.team_id.alias_id and self.team_id.alias_id.alias_name and self.team_id.alias_id.alias_domain:
                            alias_email = self.team_id.alias_id.alias_name +'@'+ self.team_id.alias_id.alias_domain
                            email_values = {'email_from': alias_email,'email_to': self.partner_id.email}
                        else:
                            email_values = {
                            'email_from': self.company_id.email_formatted, 'email_to': self.partner_id.email}
                        template.sudo().send_mail(self.id, force_send=True, email_values=email_values)
    ...
    def message_post(self, **kwargs):
        ...
        if self.team_id and self.team_id.alias_id.alias_name and self.team_id.alias_id and self.team_id.alias_id.alias_domain:

            alias_email = self.team_id.alias_id.alias_name + \
                '@'+self.team_id.alias_id.alias_domain
            outgoing_mail_server_id = self.env['ir.mail_server'].sudo().search(
                [('smtp_user', '=', alias_email), ('active', '=', True)], limit=1)

            kwargs.update({'mail_server_id': outgoing_mail_server_id.id})

            # email_formatted = tools.formataddr((self.team_head.name or u"False", outgoing_mail_server_id.smtp_user or u"False"))
            # kwargs.update({'email_from':email_formatted})
