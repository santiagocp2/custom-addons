...
class TicketDashboard(models.Model):
    ...
    def get_team_leader(self):
        ...
        team_heads = self.env['helpdesk.team'].sudo().search([]).mapped('team_head').ids
    ...
    def get_team(self,team_leader):
        if team_leader:
            teams = self.env['helpdesk.team'].sudo().search_read([('team_head','=',int(team_leader))])
            return teams

    @api.model
    def get_team_members(self,team):
        if team:
            members = self.env['helpdesk.team'].sudo().search([('id','=',int(team))]).mapped('team_members').ids
