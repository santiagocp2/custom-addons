...
class DownloadReport(http.Controller):
    ...
    def download_ticket(self,
        ...
        return self._show_report(
            ...
            report_ref='sh_all_in_one_helpdesk.action_report_helpdesk_ticket',
            download=download)
...
class HelpdeskTicketFeedbackController(http.Controller):
    ...
    @http.route('/helpdesk/ticket/feedback/<ticket_id>',
                type="http",
                auth="public",
                website=True,
                csrf=False)
    ...
    def get_team_leader_data(self):
        team_heads = request.env['helpdesk.team'].sudo().search(
            []).mapped('team_head')
        res_list = {}
        if team_heads:
