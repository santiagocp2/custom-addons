        ...
        try:
            ...
            if login_user and login_user.login != 'public':
                ...
                if partner_id:
                    ...
                    if not ticket_dic.get('team_id') or not ticket_dic.get(
                        ...
                        if login_user.sh_portal_user_access and request.env.user.has_group(
                            ...
                            if request.env.company.sh_default_team_id:
                                ticket_dic.update({
                                    'team_id':
                                    request.env.company.sudo().sh_default_team_id.id,
                                    'team_head':
                                    request.env.company.sudo().sh_default_team_id.sudo().team_head.id,
                                    'user_id':
                            ...
                            if request.env.company.sh_default_team_id:
                                ticket_dic.update({
                                    ...
                                    request.env.company.sudo().sh_default_team_id.sudo().team_head.id,
                                    'user_id':
                                    request.env.company.sudo().sh_default_user_id.id,
                                })
                            else:
                    ...
                    # ---------------------------------------------------------------------------------------------------------
                    # IF CURRENT USER IS PORTAL SUPPORT USER THAN NEED TO ADD IN ASSINGED USER WHILE DEFAULT TEAM ENABLE
                    # ---------------------------------------------------------------------------------------------------------

                    if request.env.company.sh_default_team_id and request.env.user.sh_portal_user_access:
                        ticket_dic.update({'user_id':request.env.user.id})
