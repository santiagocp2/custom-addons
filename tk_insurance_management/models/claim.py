...
class ClaimInformation(models.Model):
    ...
    insured_id = fields.Many2one(related='insurance_id.insured_id', string='Insured')
    ...
    insurance_nominee_id = fields.Many2one(related="insurance_id.insurance_nominee_id", string="Nominee")
    your_nominee_is_your = fields.Selection(related="insurance_nominee_id.your_nominee_is_your",
                                            string="Your Nominee is Your")
    nominee_dob = fields.Date(related="insurance_nominee_id.nominee_dob", string=" Date of Birth")

    insurance_policy_id = fields.Many2one(related='insurance_id.insurance_policy_id', string='Insurance Policy')
    insurance_category_id = fields.Many2one(related='insurance_id.insurance_category_id', string="Policy Category",
                                            required=True)
    insurance_sub_category_id = fields.Many2one(related='insurance_id.insurance_sub_category_id', string="Sub Category")

    insurance_time_period = fields.Char(related="insurance_policy_id.insurance_time_period_id.t_period")
    agent_id = fields.Many2one(related='insurance_id.agent_id', string='Agent', readonly=True)
    policy_amount = fields.Monetary(related="insurance_id.total_policy_amount", string="Policy Amount")
    ...
    amount_residual = fields.Monetary(related='invoice_id.amount_residual', string="Claim Amount")
    ...
    state = fields.Selection(
        [('draft', "Draft"), ('submit', "Submit"), ('approved', "Approved"), ('not_approved', "Not Approved")],
        default='draft')
